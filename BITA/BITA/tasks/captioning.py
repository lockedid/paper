...
class CaptionTask(BaseTask):
    ...
    def _report_metrics(self, eval_result_file, split_name):
        ...
        coco_gt_root = "/home/wj/dataset/RSIC/RSICD/annotations"
...
def coco_caption_eval(coco_gt_root, results_file, split):
    filenames = {
        "val": "RSICD_val_gt.json",
        "test": "RSICD_test_gt.json",
    ...
    # 修复：确保 GT 标注有 info 字段
    if "info" not in coco.dataset:
        coco.dataset["info"] = {"description": "Captions"}
    if "licenses" not in coco.dataset:
        coco.dataset["licenses"] = []
    # 修复：支持结果文件为列表格式
    result_data = json.load(open(results_file, "r"))
    
    # 判断结果格式：列表 或 字典
    if isinstance(result_data, list):
        annotations = result_data
    elif isinstance(result_data, dict) and "annotations" in result_data:
        annotations = result_data["annotations"]
    else:
        raise ValueError(f"Invalid result format: {type(result_data)}")
    
    # 直接传入列表而不是文件路径
    coco_result = coco.loadRes(annotations)
